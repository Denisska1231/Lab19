...
void importDataFromFile(string namelist, vector<string> &names, vector<int> &scores, vector<char> &grades){
    ifstream sources(namelist);
    string textline;
    char format[] = "%[^:]: %d %d %d";
...
void getCommand(string &command,string &key){
    ...
    sscanf(commandLine.c_str(), format, command2, key2);
    command = command2;
    key = key2;
}
